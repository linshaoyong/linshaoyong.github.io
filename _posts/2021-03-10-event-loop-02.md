...
event loop是[Reactor设计模式](https://en.wikipedia.org/wiki/Reactor_pattern)的一部分。Reactor的核心思想就是每个异步IO操作都对应一个handler，在Node.js这个handler就是回调函数（callback）。当IO操作触发事件，相应的回调函数进入执行队列，event loop从队列中取出callback执行。event loop是单线程的，所以callback要尽量轻量。
...
even loop开始后，一次进入每个phase，开始从当前phase获取callbacks前，如果process.nextTick callbacks和Promise callbacks不为空，先执行这两个队列的callbacks，再执行当前phase的callbacks，然后进入下一个phase。
